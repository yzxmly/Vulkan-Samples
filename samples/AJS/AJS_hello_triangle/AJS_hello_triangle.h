 ...
 */
class HelloTirangle : public vkb::Application
{
<<<<<<< HEAD
...
<<<<<<< HEAD
	...
	struct Context
    {
		...
		std::vector<VkSemaphore> recycledSemaphore;
...
<<<<<<< HEAD
	...
	bool validate_layer(const std::vector<const char *>      &required,
	...
	void init_per_frame(Context &context, PerFrame &perFrame);

	void teardown_per_frame(Context &context, PerFrame &perFrame);
=======
	bool validate_extensions(const std::vector<const char *> &         required,
	                         const std::vector<VkExtensionProperties> &available);
...
<<<<<<< HEAD
	...
	std::unique_ptr<vkb::Instance> mVkInstance;
};
